...
"""
Zero-inflated factor analysis (ZIFA). Performs dimensionality reduction on
zero-inflated data.
...
where Y is the observed zero-inflated data, k is the desired number of latent
dimensions, and Z is the low-dimensional projection.
...
def checkNoNans(matrix_list):
    ...
    for i, M in enumerate(matrix_list):
        if np.any(np.isnan(np.array(M))) or np.any(np.isinf(np.array(M))):
            raise Exception(('Matrix index %i in list has a NaN or ' +
                             'infinite element') % i)


def invertFast(A, d):
    """
    given an array A of shape d x k and a d x 1 vector d,
    computes (A * A.T + diag(d)) ^{-1}
    ...
    M = np.diag(d_inv) - inv_d_squared \
        * np.dot(np.dot(A, np.linalg.inv(np.eye(k, k) +
                 np.dot(A.T, mult_diag(d_inv, A)))), A.T)
    return M
...
def Estep(Y, A, mus, sigmas, decay_coef):
    ...
    for i in range(N):
        ...
        zero_indices = np.array([True for a in range(K)] +
                                [np.abs(Y_i[j]) < 1e-6 for j in range(D)])
        dim = K + D  # this is dimension of matrix

        # 2. compute P(Z, X_0 | Y_+)
        (mu_c, sigma_c, augmentedA_0, augmentedA_plus,
         augmented_D, sigma_22_inv) = \
            calcConditionalDistribution(A, mus, sigmas,
                                        np.array([np.abs(Y_i[j]) < 1e-6
                                                  for j in range(D)]),
                                        Y_i[~Y_is_zero])

        # 3. compute P(Z, X_0 | Y_+, Y_0)
        dim = len(sigma_c)
        matrixToInvert = computeMatrixInLastStep(A, np.abs(Y[i, :]) < 1e-6,
                                                 sigmas, K, sigma_c,
                                                 decay_coef, sigma_22_inv)
        if (Y_is_zero).sum() < D:
            magical_matrix = 2 * decay_coef * \
                             (mult_diag(augmented_D, matrixToInvert) +
                              augmentedA_0 *
                              (np.eye(K)-augmentedA_plus.T *
                               sigma_22_inv * augmentedA_plus) *
        ...
        if (Y_is_zero).sum() < D:
            sigma_xz = np.array(
                sigma_c - mult_diag(augmented_D,
                                    np.array(magical_matrix), left=False) -
                (magical_matrix * augmentedA_0) *
                ((np.eye(K) - augmentedA_plus.T *
                 sigma_22_inv * augmentedA_plus) * augmentedA_0.T))
        else:
            sigma_xz = np.array(
                sigma_c - mult_diag(augmented_D,
                                    np.array(magical_matrix),
                                    left=False) -
                (magical_matrix * augmentedA_0) * augmentedA_0.T)
        mu_xz = np.array(np.matrix(np.eye(dim) - magical_matrix) *
                         np.matrix(mu_c))
        EZ[i, :] = mu_xz[:K, 0]
        EX[i, Y_is_zero] = mu_xz[K:, 0]
        EX2[i, Y_is_zero] = mu_xz[K:, 0] ** 2 + np.diag(sigma_xz[K:, K:])
        EZZT[i, :, :] = (np.dot(np.atleast_2d(mu_xz[:K, :]),
                                np.atleast_2d(mu_xz[:K, :].transpose())) +
                         sigma_xz[:K, :K])
        EXZ[i, Y_is_zero, :] = (np.dot(mu_xz[K:], mu_xz[:K].transpose()) +
                                sigma_xz[K:, :K])
...
def computeMatrixInLastStep(A, zero_indices, sigmas, K, sigma_c, decay_coef,
                            sigma_22_inv):
    """
    ...
    if (E_plusplus_inv.shape[0] == 0) or (E_plusplus_inv.shape[1] == 0):
        ...
        inv_matrix = np.linalg.inv(2. * decay_coef *
                                   (np.linalg.inv(E_00_prime_inv) -
                                    E_0plus * E_plusplus_inv * E_0plus.T))
    else:
        b_inv = np.linalg.inv((np.matrix(A_0).T * E_00_prime_inv) *
                              np.matrix(A_0))
        innermost_inverse = applyWoodburyIdentity(-E_plusplus_inv, b_inv,
                                                  np.matrix(A_plus))
        inv_matrix = ((1 / (2. * decay_coef)) *
                      (E_00_prime_inv - (E_00_prime_inv * A_0) *
                      (A_plus.T * innermost_inverse * A_plus) *
...
def Mstep(Y, EZ, EZZT, EX, EXZ, EX2, oldA, old_mus, old_sigmas,
          old_decay_coef, singleSigma=False):
    """
    ...
    A_product = np.tile(np.reshape(A, [1, D, K]), [K, 1, 1]) \
        * (np.tile(np.reshape(A, [1, D, K]), [K, 1, 1]).T)
    for i in range(N):
...
def calcConditionalDistribution(A, mus, sigmas, zero_indices, observed_values):
    ...
    if len(observed_values) == 0:
        ...
        return (mu_x, sigma_x, augmentedA_0, augmentedA_plus, \
                augmented_D, np.array([[]]))
    sigma_22_inv = np.matrix(invertFast(A[~zero_indices, :],
                             sigmas[~zero_indices] ** 2))
...
def decayCoefObjectiveFn(x, Y, EX2):
    ...
    with warnings.catch_warnings():
        ...
        grad = sum(sum(Y_is_zero * (-EX2) +
                       (1 - Y_is_zero) * y_squared * exp_ratio))
...
def initializeParams(Y, K, singleSigma=False, makePlot=False):
    """
    initializes parameters using a standard factor analysis model
    (on imputed data) + exponential curve fitting.
    ...
    singleSigma: uses only a single sigma as opposed to a different sigma
        for every gene
    makePlot: makes a mu - p_0 plot and shows the decaying exponential fit.

    Returns: A, mus, sigmas, decay_coef: initialized model parameters.
    """
    ...
    if (mse > 0) and makePlot:
        ...
        plot(np.arange(min(means), max(means), .1),
             np.exp(-decay_coef * (np.arange(min(means),
                                   max(means), .1) ** 2)))
...
def fitModel(Y, K, singleSigma=False):
    ...
    Y: data matrix, n_samples x n_genes
    K: number of latent components
    singleSigma: if True, fit only a single variance parameter
        (zero-inflated PPCA) rather than a different one for every gene
        (zero-inflated factor analysis).
    Returns:
    EZ: the estimated positions in the latent space, n_samples x K
    params: a dictionary of model parameters. Throughout, we refer to lambda
        as "decay_coef".
    """
    N, D = Y.shape
    Y_is_all_zero = (np.abs(Y) < 1e-6).sum(axis=0) == N
    if Y_is_all_zero.sum() > 0:
        raise Exception("Your Y matrix has columns which are entirely zero; " +
                        "please filter out these columns and rerun the " +
                        "algorithm.")
    print('Running zero-inflated factor analysis with N = %i, D = %i, K = %i'
          % (N, D, K))

    # initialize the parameters
    np.random.seed(23)
    A, mus, sigmas, decay_coef \
        = initializeParams(Y, K, singleSigma=singleSigma)
    ...
    while n_iter < max_iter:
        ...
        new_A, new_mus, new_sigmas, new_decay_coef = \
            Mstep(Y, EZ, EZZT, EX, EXZ, EX2, A, mus, sigmas, decay_coef,
                  singleSigma=singleSigma)
        checkNoNans([EZ, EZZT, EX, EXZ, EX2, new_A, new_mus,
                    new_sigmas, new_decay_coef])
        paramsNotChanging = True
        max_param_change = 0
        for new, old in [[new_A, A], [new_mus, mus], [new_sigmas, sigmas],
                         [new_decay_coef, decay_coef]]:
            rel_param_change = np.mean(np.abs(new - old)) \
                               / np.mean(np.abs(new))
